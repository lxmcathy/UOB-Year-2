...
np :: Parser Tree
np = P (\inp -> case parse pronoun inp of
  [(x1,out1)] -> [(Branch NP [x1],out1)]
  otherwise -> case parse properNoun inp of
    [(x2,out2)] -> [(Branch NP [x2],out2)]
    otherwise -> (case parse determiner inp of
      [(x3,out3)] -> (case parse space' out3 of
                      [(x4,out4)] -> (case parse nominal out4 of
                                      [(x5,out5)] -> [(Branch NP ([x3]++[x5]),out5)]
                                      otherwise -> [])
                      otherwise -> [])
      otherwise -> []))
{- Exercise 2.5 -}
vp :: Parser Tree
vp = P (\inp -> case parse verb inp of
  [(x1,out1)] -> (case parse space' out1 of
    [(x2,out2)] -> (case parse (select [np,pp]) out2 of
                    [(x3,out3)] -> (case parse space' out3 of
                      [(x4,out4)] -> (case parse pp out4 of
                                      [(x5,out5)] -> [(Branch VP ([x1]++[x3]++[x5]),out5)]
                                      otherwise -> [(Branch VP ([x1]++[x3]),out3)])
                      otherwise -> [(Branch VP ([x1]++[x3]),out3)])
                    otherwise -> [(Branch VP [x1],out1)])
    otherwise -> [(Branch VP [x1],out1)])
  otherwise -> [])

{- Exercise 2.6 -}
pp :: Parser Tree
pp = P (\inp ->(case parse preposition inp of
  [(x3,out3)] -> (case parse space' out3 of
                  [(x4,out4)] -> (case parse np out4 of
                                  [(x5,out5)] -> [(Branch PP ([x3]++[x5]),out5)]
                                  otherwise -> [])
                  otherwise -> [])
  otherwise -> []))
...
{- Exercise 2.7 -}
sent :: Parser Tree
sent = P (\inp ->(case parse np inp of
  [(x3,out3)] -> (case parse space' out3 of
                  [(x4,out4)] -> (case parse vp out4 of
                                  [(x5,out5)] -> [(Branch Sentence ([x3]++[x5]),out5)]
                                  otherwise -> [])
                  otherwise -> [])
  otherwise -> []))
